            ...
            <?php foreach ($perguntas as $index => $pergunta): ?>
                <div class="pergunta" id="pergunta-<?php echo $index; ?>" style="display: <?php echo $index === 0 ? 'block' : 'none'; ?>;">
                    ...
                    <div class="botoes-resposta">
                        <?php for ($i = 0; $i <= 10; $i++): ?>
                            <button type="button" class="botao-nota" data-index="<?php echo $index; ?>" data-valor="<?php echo $i; ?>" onclick="selecionarNota(this, <?php echo $index; ?>)">
                                <?php echo $i; ?>
                            </button>
                        <?php endfor; ?>
                    </div>
                    <input type="hidden" name="resposta[<?php echo $pergunta['id_pergunta']; ?>]" id="resposta-<?php echo $index; ?>">
                    <div class="feedback-container">
                        <label>Feedback (opcional)</label>
                        <textarea name="feedback[<?php echo $pergunta['id_pergunta']; ?>]" placeholder="Deixe seu feedback..."></textarea>
                    </div>
                    <button type="button" class="confirmar-btn" onclick="proximaPergunta(<?php echo $index; ?>)" disabled>Confirmar</button>
                </div>
            <?php endforeach; ?>
        </div>
